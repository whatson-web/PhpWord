...
class Word2007 extends AbstractWriter implements WriterInterface
{
    ...
    public function __construct(PhpWord $phpWord = null)
    {
        ...
        foreach (array_keys($this->parts) as $partName) {
            ...
            if (class_exists($partClass)) {
                /** @var \WH\PhpWord\Writer\Word2007\Part\AbstractPart $part Type hint */
                $part = new $partClass();
        ...
        if (!empty($elements)) {
            foreach ($elements as $file => $media) {
                if (count($media) > 0) {
                    ...
                    /** @var \WH\PhpWord\Writer\Word2007\Part\AbstractPart $writerPart Type hint */
     ...
     *
     * @param \WH\PhpWord\Element\Section &$section
     * @param \WH\PhpWord\Shared\ZipArchive $zip
    ...
    private function addNotes(ZipArchive $zip, &$rId, $noteType = 'footnote')
    {
        ...
        /** @var \WH\PhpWord\Collection\AbstractCollection $collection Type hint */
    ...
    private function addChart(ZipArchive $zip, &$rId)
    {
        ...
        $index = 0;
        if ($collection->countItems() > 0) {
            /** @var \WH\PhpWord\Element\Chart $chart */
